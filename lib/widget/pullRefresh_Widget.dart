...
class _PullRefreshLayoutState extends State<PullRefreshLayout>
  ...
  Widget build(BuildContext context) {
    ...
    double refreshWidgetHeight =
        currentDirection == Direction.DOWN ? scrollOffset.dy : 0;
    // 上拉时，刷新Widget隐藏，加载更多Widget显示
    double loadMoreWidgetHeight =
        currentDirection == Direction.UP ? scrollOffset.dy.abs() : 0;
    ...
    return Center(
        child: dataList == null ? CircularProgressIndicator() : refreshWidget);
  }
  ...
  void animShowRefresh() {
    ...
    scrollAnimation = scrollAnimController.drive(
      Tween<double>(
        begin: scrollOffset.dy,
        end: currentDirection == Direction.DOWN
            ? refreshWidgetHeight
            : -refreshWidgetHeight,
      ),
    );
  ...
  Future<List<String>> loadMore() async {
    return await Future.delayed(Duration(seconds: 2),
        () => List.generate(5, (index) => 'add more item'));
  }

  /// 加载刷新数据
  Future<List<String>> refresh() async {
    return await Future.delayed(Duration(seconds: 2),
        () => List.generate(5, (index) => 'refresh item'));
  }
